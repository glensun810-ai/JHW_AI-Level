...
import { generateTierCardImage } from '@/utils/canvas-renderer.js';
...
async function generate() {
  ...
  const cacheKey = `${props.tierName}_${props.totalScore}`;
  ...
  try {
    ...
    const imagePath = await generateTierCardImage({
    ...
    uni.showToast({ title: '段位卡生成失败，请重试', icon: 'none' });
  } finally {
    loading.value = false;
  }
}
