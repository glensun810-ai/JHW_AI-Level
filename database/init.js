...
const SCHEMAS = {
  ...
  test_records: {
    ...
    fields: {
      ...
      answers:     { type: 'array',  required: true,  description: '[{questionId, selectedIndex, score}]' },
      totalScore:  { type: 'number', required: true,  description: '总分（5-50）' },
      tier:        { type: 'string', required: true,  description: '本次段位名称' },
      questionSetId: { type: 'string', required: true, description: '题本 ID' },
      prevTier:    { type: 'string', required: false, description: '上次段位（首次为 null）' },
      tierChanged:  { type: 'string', required: false, description: "'up' | 'down' | 'same' | null（首次为 null）" },
      createdAt:   { type: 'date',   required: false, description: '创建时间' },
    },
    indexes: [
