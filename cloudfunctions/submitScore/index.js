...
// 数据库索引初始化（首次调用时自动创建）
async function ensureWeeklyRankingsIndex() {
  try {
    const db = cloud.database();
    // 尝试创建集合（已存在则忽略）
    try {
      await db.createCollection('weekly_rankings');
    } catch (e) {
      // 集合已存在，正常
    }
    // 创建复合索引（幂等：已存在则忽略）
    try {
      await db.collection('weekly_rankings').createIndex({
        weekStart: 1,
        score: -1
      });
    } catch (e) {
    ...
    console.log('[init] weekly_rankings index ready');
  } catch (e) {
    console.log('[init] index init skipped:', e.message);
  }
}

// 索引初始化在 exports.main 首次调用时执行（避免顶层 await）
let indexInitDone = false;
async function initIndexOnce() {
  if (indexInitDone) return;
  indexInitDone = true;
  try { await ensureWeeklyRankingsIndex(); } catch (e) { /* ignore */ }
}
...
exports.main = async (event, context) => {
  try {
    // 首次冷启动时自动创建索引（在 try 块内，失败不影响主流程）
    await initIndexOnce();
