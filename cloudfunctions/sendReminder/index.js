...
async function sendCheckinReminders(today) {
  const CHECKIN_TEMPLATE_ID = process.env.CHECKIN_TEMPLATE_ID || '';
  if (!CHECKIN_TEMPLATE_ID) {
    console.log('[sendReminder] 未配置 CHECKIN_TEMPLATE_ID，跳过签到提醒');
    return { code: 0, message: '未配置模板ID', data: { sent: 0, total: 0 } };
  }
  ...
  for (const user of users) {
    ...
    const checkinSub = subs.find(s =>
      s.templateId === CHECKIN_TEMPLATE_ID && s.status === 'accept'
    );
    ...
    try {
      await cloud.openapi.subscribeMessage.send({
        ...
        data: {
          thing9: { value: '你今天还没签到哦，断签会重置连续天数' },
          number6: { value: String(user.consecutiveDays) },
          number8: { value: '0' },
          thing7: { value: '未签到' },
          thing2: { value: '进入进化湾测测今日AI段位' },
        },
...
async function sendTestReminders(today) {
  const TEST_REMINDER_TEMPLATE_ID = process.env.TEST_REMINDER_TEMPLATE_ID || '';
  if (!TEST_REMINDER_TEMPLATE_ID) {
    console.log('[sendReminder] 未配置 TEST_REMINDER_TEMPLATE_ID，跳过测试提醒');
    return { code: 0, message: '未配置模板ID', data: { sent: 0, total: 0 } };
  }
  ...
  for (const user of users) {
    const subs = user.subscribeTemplates || [];
    const testSub = subs.find(s =>
      s.templateId === TEST_REMINDER_TEMPLATE_ID && s.status === 'accept'
    );
    if (!testSub) continue;
    ...
    try {
      await cloud.openapi.subscribeMessage.send({
        ...
        data: {
          thing1: { value: 'AI段位测试' },
          thing2: { value: isDormant ? '你已经一周没来进化湾了' : `${daysSince}天没测段位了` },
          time8: { value: new Date(user.updatedAt).toISOString().slice(0, 10) },
          thing14: { value: isDormant ? '你的AI段位可能已经发生变化，快来重新测试' : '坚持测试，追踪你的AI段位进化' },
        },
